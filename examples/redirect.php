...
// Retrieve the stored state and PKCE verifier from the session
    $storedState = $_SESSION['oauth2_state'] ?? null;
...
// Validate that the returned state matches the stored state
    ...
    try {
        $asanaClient->handleCallback($authorizationCode, $pkceVerifier);
        // Token is saved automatically by TokenManager -> TokenStorage
